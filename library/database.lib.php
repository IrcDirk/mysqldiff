 ...
 *
 *    $Author: sskus $
 *    $RCSfile: database.lib.php,v $
 *    $Revision: 1.22 $
 *    $Date: 2004/09/29 14:36:01 $
 *    $State: Exp $
 *
 * ------------------------------------- */
...
define("CMDBH_DML", "dml");
define("CMDBH_DDL", "ddl");
define("CMDBH_TYPE", "type");
define("CMDBH_CONSTANTS", "const");
define("CMDBH_VALUES", "values");
define("CMDBH_NUMBER", "num");
define("CMDBH_SIGNS", "signs");
define("CMDBH_OBJECT", "obj");

define("CONSTRAINT_ADD", 0);
define("CONSTRAINT_DROP", 1);
define("CONSTRAINT_CREATE", 2);
...
class cConnection {
	...
	function open($persistent = FALSE) {
		static $connectionfunctions = array(1 => "mysqli_connect", 0 => "mysqli_connect");
	...
	function importSql($script) {
		$incmd = FALSE;
		$items = $this->splitSqlFile($script);
		$success = $failures = 0;
		foreach ( $items As $item ) {
			if ( trim($item) != "" ) {
				if ( mysqli_query($item, $this->_con) ) {
					$success++;
				} else {
					echo "ERROR[".mysqli_errno($this->_con)."] ".mysqli_error($this->_con)."\n$item\n\n";
					$failures++;
				}
			}
		}
		return array("statements" => count($items), "success" => $success, "failures" => $failures);
	}

	function listDatabases($numericindex = FALSE) {
		$result = array();
		if ( $res = mysqli_query($this->_con, "SHOW DATABASES") ) {
	...
	function selectDatabase($name) {
		if ( !mysqli_select_db($this->_database = $name, $this->_con) ) {
			$this->_database = NULL;
		}
	...
	function createTemporaryDatabase($name = NULL) {
		...
		$idx++;
		mysqli_query("CREATE DATABASE $tempname", $this->_con);
		echo mysqli_error($this->_con);
		if ( mysqli_errno($this->_con) == 0 ) {
			return $tempname;
		} else return "";
	}

	function dropDatabase($name) {
		mysqli_query("DROP DATABASE $name", $this->_con);
		return mysqli_errno($this->_con) == 0;
	}
	...
	function fetchTablelist($db = NULL, $numericindex = FALSE, $extendeddisplay = FALSE) {
		...
		if ( $extendeddisplay ) {
			...
			if ( isset($db) && $res = mysqli_query($this->_con, "SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA LIKE '$db'") ) {
	...
	function serverVersion() {
		if ( preg_match("/^((\d+)\.(\d+)\.(\d+))/", mysqli_get_server_info($this->_con), $matches) ) {
...
class cQuery {
	...
	function cQuery($stat, &$parent, $file=NULL, $line=NULL) {
		...
		$this->_parent = &$parent;
		if ( $this->_res = mysqli_query($stat, $this->_parent->_con) ) {
			$this->currow = 0;
			$numrows = $this->_res->num_rows;
	...
	function destroy() {
		if ( isset($this->_res) && $this->_res!="" ) mysqli_free_result($this->_res);
	}

	function created() { return isset($this->_res); }

	function count() { return $this->_res->num_rows; }

	function nextarray($type=SQL_BOTH) {
		if ( $this->_res ) $this->_rowarray = $this->_res->fetch_array($type);
		else $this->_rowarray=NULL;
		return isset($this->_rowarray) ? $this->_rowarray : NULL;
	}

	function next() {
		if ( $this->_res ) $this->_row = $this->_res->fetch_object();
		else $this->_row=NULL;
...
class cCommandBuilder {
	...
	*/
	...
	function alterTableContraints($source, $target) {
		...
		if ( $this->getOption("cfk_back") && isset($source["constraints"]) ) foreach ( $source["constraints"] AS $vk=>$vf ) {
			if ( !isset($target["constraints"][$vk]) ) {
				if ( $this->getOption("short") ) {
					$altering.=($altering==""?"":$this->_translate(",")."\n")."    ".$this->_constraintString($vf, $target["database"], CONSTRAINT_DROP, $this->_con->serverVersionString());
				} else $result .= $this->_highlightString("ALTER TABLE")." ".$this->_objectName($target["name"])." ".$this->_constraintString($vf, $target["database"], CONSTRAINT_DROP, $this->_con->serverVersionString()).$this->_translate(";")."\n";
				$altered++;
			}
		}
		if ( $this->getOption("cfk_back") && isset($target["constraints"]) ) foreach ( $target["constraints"] AS $vk=>$vf ) {
			if ( !isset($source["constraints"][$vk]) ) {
				if ( $this->getOption("short") ) {
					$altering.=($altering==""?"":$this->_translate(",")."\n")."    ".$this->_constraintString($vf, $target["database"], CONSTRAINT_ADD);
				} else $result .= $this->_highlightString("ALTER TABLE")." ".$this->_objectName($target["name"])." ".$this->_constraintString($vf, $target["database"], CONSTRAINT_ADD).$this->_translate(";")."\n";
				$altered++;
			}
		}
	...
	function alterTable($source, $target) {
		...
		if ( isset($target["idx"]) ) foreach ( $target["idx"] AS $vk=>$vf ) {
			if ( !isset($source["idx"][$vk]) ) {
				...
				} else $result .= $this->_highlightString("ALTER TABLE")." ".$this->_objectName($target["name"])." ".$this->_highlightString("ADD")." ".$this->_indexString($vf).translate(";")."\n";
				$altered++;
			}
		}
		...
		if ( $this->getOption("charset") ) {
			if ( $source["collate"]!=$target["collate"] ) {
				...
				$charset = $this->_highlightString("DEFAULT CHARSET").$this->_translate("=").$this->_highlightString($charsetinfo[0], CMDBH_CONSTANTS)." ".$this->_highlightString("COLLATE").$this->_translate("=").$this->_highlightString($target["collate"], CMDBH_CONSTANTS);
		...
		if ( $this->getOption("type") ) {
			if ( $source["type"]!=$target["type"] ) {
				if ( $this->getOption("short") ) {
					$tableoptions .= ($tableoptions == "" ? ( $altering == "" ? "    " : "" ) : $this->_translate(" ")).$this->_highlightString("TYPE").$this->_highlightstring("=", CMDBH_SIGNS).$this->_highlightstring($target["type"], CMDBH_CONSTANTS);
				} else $result .= $this->_highlightString("ALTER TABLE")." ".$this->_objectName($target["name"])." ".$this->_highlighString("TYPE").$this->_highlightstring("=", CMDBH_SIGNS).$this->_highlightstring($target["type"], CMDBH_CONSTANTS).$this->_translate(";")."\n";
				$altered++;
			}
		}
		...
		if ( $this->getOption("options") ) {
			if ( $source["options"]!=$target["options"] ) {
				...
				} else $result .= $this->_highlightString("ALTER TABLE")." ".$this->_objectName($target["name"])." ".$this->_highlightString($target["options"], CMDBH_VALUES).$this->_translate(";")."\n";
				$altered++;
			}
		}
		if ( $this->getOption("auto_incr") ) {
			if ( $source["auto_incr"] != $target["auto_incr"] ) {
				if ( $this->getOption("short") ) {
					$tableoptions .= ($tableoptions == "" ? ( $altering == "" ? "    " : "" ) : $this->_translate(" ")).$this->_highlightString("AUTO_INCREMENT").$this->_highlightstring("=", CMDBH_SIGNS).$this->_con->escapestring($target["auto_incr"]);
				} else $result .= $this->_highlightString("ALTER TABLE")." ".$this->_objectName($target["name"])." ".$this->_highlightString("AUTO_INCREMENT").$this->_highlightstring("=", CMDBH_SIGNS).$this->_con->escapestring($target["auto_incr"]).$this->_highlightstring(";", CMDBH_SIGNS)."\n";
				$altered++;
			}
		}
		if ( $this->getOption("comment") ) {
			if ( $source["comment"]!=$target["comment"] ) {
				if ( $this->getOption("short") ) {
					$tableoptions .= ($tableoptions == "" ? ( $altering == "" ? "    " : "" ) : $this->_translate(" ")).$this->_highlightString("COMMENT").$this->_highlightstring("=", CMDBH_SIGNS)."'".$this->_con->escapestring($target["comment"]).$this->_translate("'");
				} else $result .= $this->_highlightString("ALTER TABLE")." ".$this->_objectName($target["name"])." ".$this->_highlightString("COMMENT").$this->_highlightstring("=", CMDBH_SIGNS)."'".$this->_con->escapestring($target["comment"]).$this->_translate("';")."\n";
				$altered++;
			}
		}
		if ( $tableoptions != "" ) {
	...
	function createTable($table) {
		...
		if ( isset($table["constraints"]) ) {
			...
			foreach ( $table["constraints"] AS $vk=>$vf ) {
				$item .= "    ".$this->_constraintString($vf, $table["database"], CONSTRAINT_CREATE).( $idx < $max ? "," : "" )."\n";
				$idx++;
			}
		}
		...
		if ( $this->getOption("type") ) {
			if ( isset($table["type"]) && $table["type"] != "" ) {
				$item .= " ".$this->_highlightString("TYPE").$this->_highlightstring("=", CMDBH_SIGNS).$this->_highlightstring($table["type"], CMDBH_CONSTANTS);
			}
		}
		if ( $this->getOption("options") ) {
		...
		if ( $this->getOption("charset") ) {
			if ( isset($table["collate"]) && $table["collate"] != "" ) {
				...
				$charset = $this->_highlightString("DEFAULT CHARSET").$this->_translate("=").$this->_highlightString($charsetinfo[0], CMDBH_CONSTANTS)." ".$this->_highlightString("COLLATE").$this->_translate("=").$this->_highlightString($table["collate"], CMDBH_CONSTANTS);
		...
		if ( $this->getOption("comment") ) {
			if ( isset($table["comment"]) && $table["comment"] != "" ) {
				$item .= " ".$this->_highlightString("COMMENT").$this->_highlightstring("=", CMDBH_SIGNS)."'".$this->_con->escapestring($table["comment"]).$this->_translate("'");
			}
		}
		$item .= $this->_translate(";");
		return $item;
	...
	function setMySqlVariable($variable, $value) {
		return $this->_highlightString("SET ", CMDBH_DML).$this->_highlightString($variable, CMDBH_OBJECT).$this->_highlightString(" = ", CMDBH_SIGNS).( is_numeric($value) ? $this->_highlightString($value, CMDBH_NUMBER) : $this->_highlightString("'$value'", CMDBH_VALUE) ).$this->_highlightString(";", CMDBH_SIGNS);
	}
	...
	*/
	...
	function _constraintString($idx, $targetdb, $what = CONSTRAINT_ADD, $serverversion = NULL) {
		...
		} else if ( $what == CONSTRAINT_DROP && isset($serverversion) && $this->_con->serverVersionCompare("4.0.13") >= 0 ) {
	...
	function _fieldString($field, $withname=TRUE) {
		...
		$result .= " ".$this->_highlightString(( $field["null"] ? "" : "NOT " )."NULL", CMDBH_CONSTANTS);
		$result .= " ".$this->_highlightString("DEFAULT", CMDBH_DDL);
		if ( isset($field["default"]) ) {
			$result .= " ".$this->_highlightstring("'".$field["default"]."'", CMDBH_VALUES);
		} else {
			$result .= " ".($field["null"] ? $this->_highlightString("NULL", CMDBH_CONSTANTS) : $this->_highlightstring($this->_alternateNullDefault($field["type"]), CMDBH_VALUES));
		}
		if ( isset($field["comment"]) && $this->_con->serverVersionCompare("4.1.0") >= 0 ) {
			$result .= " ".$this->_highlightString("COMMENT")." ".$this->_highlightString("'".$field["comment"]."'", CMDBH_VALUES);
		}
		if ( $this->getOption("charset") && isset($field["collate"]) && $this->_con->serverVersionCompare("4.1.0") >= 0 ) {
			$result .= " ".$this->_highlightString("COLLATE")." ".$this->_highlightString($field["collate"], CMDBH_CONSTANTS);
		}
	...
	function _highlightString($what, $kind = CMDBH_DDL) {
		if ( $this->_highlight ) {
	...
	function _insertreplaceRecord($table, $data, $what = "INSERT") {
		...
		return $this->_highlightString("$what INTO", CMDBH_DML)." ".$this->_objectName($table)." ".$this->_translate("(").$fields.$this->_translate(") ").$this->_highlightString("VALUES", CMDBH_DML).$this->_translate(" (").$values.$this->_translate(");");
	}

	function _objectName($name) {
		return $this->_highlightString($this->getOption("backticks") || preg_match("/[^a-z0-9_$]/i", $name) || in_array(strtoupper($name), $this->_reservedwords) ? "`".$name."`" : $name, CMDBH_OBJECT);
	}
